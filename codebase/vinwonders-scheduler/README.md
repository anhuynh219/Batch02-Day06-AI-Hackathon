...
1. `npm install`
2. Copy `.env.example` → `.env`, điền `GEMINI_API_KEY` (lấy tại [aistudio.google.com](https://aistudio.google.com/app/apikey) — key bắt đầu bằng `AIza`).
3. `npm run dev` (chạy đồng thời web + api).
4. Mở URL Vite in ra (mặc định [http://localhost:5173](http://localhost:5173)).
...
- Gemini (qua proxy `server/`) hiểu ngôn ngữ tự nhiên + CHỌN trò chơi bằng id từ dataset; không tự tính giờ.
